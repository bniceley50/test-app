...
- [x] Web end-to-end verification (all screens + a full Mock Exam cycle). *(2026-08-23, extended twice: `node tools/e2e-chrome.cjs` — real Chrome over CDP, fresh profile (exercises first-boot seeding), pointer-event taps, CDP `Input.insertText` for typing (RN-web controlled TextInput ignores synthetic events — lessons #10/#11/#12). 24/24 gates pass: home render + 41-Q seed; mock setup → live exam → answer/advance with no feedback → submit → per-topic results; home stats update after mock; bookmarks empty state; full 10-Q Drill to Results; bookmark star from drill answer card; bookmark entry expands; note typed + saved on blur (📝 hint); FRESH RELOAD → app reboots clean and study stats + bookmark + note persist in sqlite; Missed via home card; Topics grid; **Topics tap → topic-filtered drill deck (law_licensing → 1/9)**; Code list + 090 expansion; **Code search typing filters list** ("storm" → only 20:130, 090 + 070 drop out). Screenshots `01`–`07` in tasks/evidence/.)*
