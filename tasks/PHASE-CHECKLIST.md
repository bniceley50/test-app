...
- [x] Web end-to-end verification (all screens + a full Mock Exam cycle). *(2026-08-23, extended: `node tools/e2e-chrome.cjs` — real Chrome over CDP, fresh profile (exercises first-boot seeding), pointer-event taps, CDP `Input.insertText` for typing (RN-web controlled TextInput ignores synthetic events — lessons #10/#11/#12). 22/22 gates pass: home render + 41-Q seed; mock setup → live exam → answer/advance with no feedback → submit → per-topic results; home stats update after mock; bookmarks empty state; full 10-Q Drill to Results; bookmark star from drill answer card; bookmark entry expands; note typed + saved on blur (📝 hint); FRESH RELOAD → app reboots clean and study stats + bookmark + note persist in sqlite; Missed via home card; Topics grid with %; Code list + 090 expansion. Screenshots `01`–`07` in tasks/evidence/.)*
