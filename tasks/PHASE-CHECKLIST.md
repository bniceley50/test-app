...
- [x] **Code Reference** (`app/(tabs)/code.tsx`): search; expandable summary + text; section → its questions; bookmark. *(built 2026-08-18: 250ms-debounced search over title/summary/keywords, expandable cards with keyword chips + linked questions (prompt/answer/explanation), per-section and per-question bookmark stars → new `bookmarks` table, empty + error states. Round 17: `getQuestionsForCodeSection` now honors `verified = 1` (see P4 consistency audit).)*
...
## Sign-off
- [ ] **Web click-through on http://localhost:8081** — owner runs the §C* web path (PHONE-ONBOARDING.md) top to bottom: home→drill→missed→mock→topics→code→bookmarks(+note+reload-persist).
- [ ] All screens working: Home, Drill, Missed, Mock, Topics, Code Reference, Bookmarks. *(code + bundle-verified all 3 targets 2026-08-18; live-server + E2E-verified; awaiting owner's phone pass on the new screens.)*
...
- **`verified=1` in every study-fetch path.** Re-verified all 7 question fetches
  in `lib/database.ts`. Six already filtered `verified=1` (drill deck, due
  blend ✓, topic, mock, code-section-count, fill-blank). **One gap found &
  fixed:** `getQuestionsForCodeSection` did NOT filter, so an unverified row
  linked to a code section would render in the Code Reference expansion even
  though the verified-only count query next to it and the "No verified
  questions…" copy promised otherwise. Added `AND verified = 1`. A new db-smoke
  check now inserts an unverified row into `815KAR20:090` and asserts the
  expanded fetch (12) and the section count (12) agree while unfiltered (13) —
  23 smoke checks green.
- **Unified loading / empty / error states — audited, not assumed.** mock /
  drill / missed / bookmarks / code / topics all have distinct `loading` +
  `error` states (error = red card + actionable message); every list has an
  empty state (No bookmarks yet / No matches / No missed questions / No
  questions linked / No topics); root `_layout` exports `ErrorBoundary`;
  `results` is pure derived state (params in — no fetch, no unguarded async),
  so it needs no loading/error/empty. Home uses store-cached stats + an error
  card with "close & reopen" retry.
