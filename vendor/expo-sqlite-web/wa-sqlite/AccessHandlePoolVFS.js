...
export class AccessHandlePoolVFS extends FacadeVFS {
  ...
  async isReady() {
    if (!this.#directoryHandle) {
      ...
      // DSH patch (test-app): pool capacity may be below DEFAULT_CAPACITY not
      // only on first boot (nothing on disk) but also when some pool files
      // were skipped above because another context still holds them. Top up
      // by the gap in that case too, so jOpen still has room to place the
      // main DB / journal / wal files.
