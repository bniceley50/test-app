...
async function maybeInitAsync(): Promise<{
  ...
  // DSH patch (test-app): a web full-page navigation can race the previous
  // (still-open, garbage) document's OPFS sync access handles, so the
  // persistent VFS create can fail in this worker's first attempt. That used
  // to leave `_vfs == null` after `_sqlite3` was set — a permanently
  // poisoned worker reporting "Invalid VFS state" on every later open.
